...
> 글보다 그림이 빠릅니다. 아래 화면들을 먼저 보면 이 프로젝트가 무엇인지 바로 이해됩니다.

---

### 1️⃣ 환자 정보를 입력하면 입원일수를 예측해줍니다

![React Web UI 예측 결과](docs/images/react-ui-prediction.png)

**이게 핵심입니다.** 왼쪽에 환자 정보(나이, 성별, 병명 등)를 입력하고 버튼을 누르면, AI가 오른쪽에 **예상 입원일수**를 바로 알려줍니다.

- 위 화면: 48세 여성, 폐렴(pneumonia), 검사 이상 있음 → **예상 4.8일 (5일)**
- 상단에 ✅ 서버 정상 / 현재 사용 중인 AI 모델(GradientBoostingRegressor) 표시
- 웹 브라우저에서 바로 사용 가능 (설치 불필요)

---

### 2️⃣ 4가지 AI 모델을 자동으로 비교해서 가장 좋은 것을 선택합니다

![MLflow 실험 실행 목록](docs/images/mlflow-runs.png)

**모델을 하나만 쓰지 않습니다.** XGBoost, GradientBoosting, RandomForest, DecisionTree — 4가지를 동시에 학습하고, 성능을 자동으로 비교해 가장 정확한 모델을 선택합니다.

- 각 줄이 하나의 모델 학습 실험 기록입니다
- 학습 시간, 성능 지표(RMSE 등), 사용한 파라미터가 모두 기록됩니다
- **MLflow** 라는 도구가 이 기록을 자동으로 관리합니다

---

### 3️⃣ 학습된 모델은 버전별로 관리됩니다

![MLflow 모델 레지스트리](docs/images/mlflow-models.png)

학습이 끝난 모델은 **모델 레지스트리**에 저장됩니다. 마치 앱 스토어처럼 버전별로 관리되어, 이전 모델로 되돌리거나 새 모델로 교체하기 쉽습니다.

- `pipeline` 태그: 데이터 전처리 + 예측 모델이 하나로 묶인 패키지
- 실제 서비스에서는 이 레지스트리에서 모델을 꺼내 배포합니다

---

### 4️⃣ API 요청 수와 응답 시간을 실시간으로 모니터링합니다

![Grafana API 요청 현황 대시보드](docs/images/grafana-api-dashboard.png)

**서비스가 잘 돌아가고 있는지** 실시간으로 확인하는 화면입니다. Grafana 라는 모니터링 도구로 시각화합니다.

- **670** — 지금까지 처리한 총 예측 요청 수
- **0** — 에러 없이 모두 정상 처리됨
- **50ms** — API 평균 응답 시간 (0.05초)
- 그래프: 시간별 요청 추이, 병명별 분포, 예측 입원일수 분포 등

---

### 5️⃣ 서버의 CPU·메모리 사용량까지 자동으로 수집됩니다

![Grafana 시스템 리소스 대시보드](docs/images/grafana-system-resources.png)

**서버가 힘들어하고 있지는 않은지** 확인하는 화면입니다. Kubernetes 위에서 돌아가는 앱의 리소스를 자동으로 수집합니다.

- 초록색 **모델 상태 정상** — AI 모델이 정상적으로 메모리에 올라가 있음
- **GradientBoostingRegressor** — 현재 서비스 중인 모델 이름
- Node CPU/메모리: Kubernetes를 구성하는 서버(노드) 4대의 자원 사용량
- Pod CPU/메모리: 병원 예측 앱 컨테이너의 자원 사용량
